...
struct BG_COMPRESSION_THREAD : BackgroundThread {
  ...
  char *dict;
  STAT *aux_statbuf;
  TIndex *hashtable[BUFFERS]; // Place for saving info about maximum hashes and
                              // their indexes
  char *bufptr[BUFFERS];
  char *buf[BUFFERS];
  ...
  BG_COMPRESSION_THREAD(bool _ROUND_MATCHES, bool _COMPARE_DIGESTS,
                        unsigned _BASE_LEN, unsigned dict_min_match,
                        bool _no_writes, hash_func_t _hash_func,
                        void *_hash_obj, Offset _filesize,
                        Offset inmem_dictsize, unsigned _bufsize,
      ...
      : errcode(NO_ERRORS), k(0), ROUND_MATCHES(_ROUND_MATCHES),
        ...
        fstat(_fstat) {
    dictsize = roundUp(inmem_dictsize, bufsize) +
               BUFFERS * bufsize; // Dictionary size should be divisible by
                                  // bufsize and has additional BUFFERS*bufsize
                                  // space reserved for background I/O
    dict = (char *)BigAlloc(dictsize, LargePageMode);

    size_t aux_statbuf_size =
        sizeof(STAT) *
        (inmem_dictsize == 0
             ? STATS_PER_MATCH(ROUND_MATCHES)
             : MAX_STATS_PER_BLOCK(bufsize, dict_min_match) + 10);
    aux_statbuf = (STAT *)malloc(
        aux_statbuf_size); // We need to store only one "fence" match if there
                           // is no in-memory compression involved

    size_t hashtable_size =
        (inmem_dictsize == 0
             ? 1
             : sizeof(TIndex) * (bufsize / inmem.L + INMEM_PREFETCH) *
                   2); // For every L bytes in the buffer, we need 2 hash table
                       // elements
    size_t statbuf_size =
    ...
    memreqs = dictsize + aux_statbuf_size +
              BUFFERS * (hashtable_size + statbuf_size + header_size);
  }
